...
// 真实后端采集:每个场景一个新 session,跑完读 window.__sseDump 落盘到 golden-master/。
// 采完把文件移到 src/features/chat/sse/__fixtures__/(Step 3 建)。
// WS = agent 工作目录(selectDirectory 桩返回值)。默认用仓内 capture/workspace(含 topology.txt/
// ip-plan.txt 等场景所需文件);要换目录用 CAPTURE_WS 环境变量覆盖。
const WS = process.env.CAPTURE_WS ?? resolve(process.cwd(), 'capture/workspace')
const OUT = 'golden-master'
...
async function startSession(page: Page, prompt: string) {
  ...
  await page.getByText('点击选择目录…').click() // 触发 selectDirectory 桩
...
test('05-retry-fail', async ({ page }) => {
  // 需后端用**坏 key + 快速失败 env** 启动(见采集指南),否则采不到失败/重试帧。
  await prep(page)

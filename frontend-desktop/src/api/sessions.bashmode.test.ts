...
describe('bash review mode api', () => {
  ...
  it('setBashReviewMode accepts strict-auto', async () => {
    await sessionsApi.setBashReviewMode('s1', 'strict-auto')
    expect(fetch).toHaveBeenCalledWith(
      '/api/v1/sessions/s1/bash-review-mode',
      expect.objectContaining({ method: 'PUT', body: JSON.stringify({ mode: 'strict-auto' }) }),
    )
  })
})

...
export function PdfViewer({ path, filename, reloadToken }: { path: string; filename: string; reloadToken?: number }) {
  const raw = useRawUrl()
  ...
  useEffect(() => {
    ...
    // 兜底看门狗:个别大 PDF / viewer 初始化异常时 pagesinit 可能迟迟不触发 → 无限转圈。
    // 12s 后强制结束 loading,把已渲染的页面显示出来。pagesinit 正常触发时会清掉它。
    let readyWatchdog: ReturnType<typeof setTimeout> | null = setTimeout(() => {
      readyWatchdog = null
      if (!destroyed) setReady(true)
    }, 12000)
    const clearReadyWatchdog = () => { if (readyWatchdog) { clearTimeout(readyWatchdog); readyWatchdog = null } }
    ...
    eventBus.on('pagesinit', () => {
      ...
      // 容器此刻可能尚无尺寸 → page-width 计算异常;不让它阻断 setReady(ResizeObserver 会重新 fit)。
      try { viewer.currentScaleValue = 'page-width' } catch { /* re-fit on resize */ }
      setReady(true)
    })
    ...
    fetchOrThrow(raw(path, reloadToken))
      ...
      .catch((e) => { clearReadyWatchdog(); if (!destroyed) setError(String(e)) })
